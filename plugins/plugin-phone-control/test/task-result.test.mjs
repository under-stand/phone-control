    ...
    run() {
      const result = deriveTaskResult({
        ...
        events: [
          ...
          { eventId: "done-turn-1", kind: "turn_complete", turnId: "turn-1", at: "2026-09-04T00:00:04Z" },
        ],
      });
      assert.equal(result.status, "completed");
      assert.equal(result.turnId, "turn-1");
      assert.equal(result.completionEventId, "done-turn-1");
  ...
  },
  {
    ...
    run() {
      const results = deriveTaskResults({ events: [
        ...
        { eventId: "done-old", kind: "turn_complete", turnId: "old", at: "2026-09-03T00:00:02Z" },
        ...
        { eventId: "done-new", kind: "turn_complete", turnId: "new", at: "2026-09-04T00:00:02Z" },
      ] });
      assert.deepEqual(results.map((result) => result.turnId), ["old", "new"]);
      assert.deepEqual(results.map((result) => result.completionEventId), ["done-old", "done-new"]);
