...
`session-projection.mjs` 是 HTTP 快照、详情和 SSE 共用的任务投影入口。它分别返回最新指令、
执行中的指令和仍待处理的指令；收件箱优先保留未确认投递，不会因为后来的指令完成而隐藏旧问题。
前端只消费完整 Session 投影，不根据单条 outbox 通知重新推导队列状态。

`session-input-service.mjs` 管理现场输入的校验、图片消费和回执；`command-replay.mjs` 在执行前
持久化请求摘要，并合并并发重试。同一消息标识的会话、设备、正文或运行参数改变时返回冲突，
已确认请求返回原结果，服务重启时未确认的请求保持待核对，不自动重发。HTTP 重试先查询原回执，
因此新一轮已经开始、现场权限状态已经变化也不会误发第二遍。底层桥接器独立保护自己的调用边界。

CLI 原生队列继续由原 CLI 消费。`keyed-operations.mjs` 只串行同一会话的修改，后台核对最多同时
处理三个会话；`atomic-json-file.mjs` 独立串行磁盘快照。失败核对采用退避，每次检查最新历史及
最多一页旧历史，逐步找回长期断线期间的回执。已完成的 CLI 展示记录达到上限后去除正文并归档，
保留消息 ID、摘要和终态用于去重，未确认消息不归档。去重记录目前仍使用本机 JSON 快照，
后续长期存储优化应保留这些语义，不能通过清空记录让旧请求再次执行。

这些边界使用显式参数连接，Codex 指令与浏览器动作继续保留各自的所有权与生命周期。
...
会话列表不再用最后一条用户消息充当标题。SessionStore 会保留第一条有实际内容的用户目标，并在后台让 Codex
生成一个持久化的智能名称；生成完成前使用本地候选；“好的，
去做吧”一类纯确认不会抢占任务身份，后续追加指令也不会改写已经确定的主目标。公共会话摘要派生出稳定
标题、目标、当前进展、最近结果和是否需要用户处理，卡片与目标追踪共用这组字段。详情响应还会为每个已结束轮次提供不含最终回复正文的附带元数据，交给对应轮次的折叠卡展示。
...
事件归组：默认只展开每轮最终回复，工具与生命周期事件保留在轮次内的原生 `details` 中；完成轮次的
结构化结果会在滚动事件窗口之外保留有上限的快照，并优先用完成事件 ID、再用轮次 ID和时间顺序绑定，
避免长会话只显示最新一张附带信息卡；最近三轮
