...
- 首页提供按紧急程度排序的行动收件箱；回答、审批、失败和送达状态不明的任务会优先出现。
- 手机指令统一显示排队、等待条件、发送、已接收、执行中、等待用户、完成、失败和需要确认等生命周期。
- 每个已结束轮次都可在对应的最终回复后附带一张默认折叠的信息卡，仅补充该轮涉及的文件、验证命令、运行记录和警告，不重复展示最终结论。
- 卡片名称默认由 Codex 根据最近几轮自动概括；生成期间先显示稳定的本地候选，完成后立即平滑替换，不需要刷新浏览器。智能命名按需处理当前列表可见的前 12 个用户会话，默认优先最近任务；切换到历史、全部或搜索结果后，对应可见的历史会话也会生成。成功的智能名称会持久化并在服务重启后保留，临时失败会保留本地候选并在后续列表刷新时重试。详情里的“智能生成”只生成可编辑候选，点“保存”后才会改名。仍可在详情中手动改名或恢复自动名称；会话主题与当前任务保持区分。
- 页面刷新或从手机后台回来时，会先使用浏览器本地保存的最近会话摘要快速绘制列表，再由实时流和 HTTP 快照校正；缓存只包含列表摘要，不包含完整对话，并会明确标记为正在同步，避免把旧状态当成实时状态。
- 按当前任务、会话主题、项目、机器、用户指令或 Codex 回复搜索已保留的任务历史。
- 在可验证的会话中继续对话、追加当前 turn、停止任务、新建、在手机与桌面应用之间显式移交控制或永久删除会话；创建会话或开始下一轮时可单独选择权限。
- 手机连接中断、电脑接管或 turn 尚未就绪时，可以先把文字指令加入“手机续作队列”；连接恢复后会自动重试，检测到 turn 已变化则停下来请你确认。
- 在会话详情点“分叉继续”，可携带最近历史创建一个独立的新会话，让手机端在原会话暂时不能继续时换一条安全路径推进。
...
- 安装可选的 Chrome Browser Bridge 后，从手机查看并操作电脑上已打开的普通网页标签页，包括切换标签、导航、点按、滚动和中文输入；开启“实时画面”后可连续观看网页视频画面。
...
Codex 0.154.0 及更新版本的原 CLI 会话提供“发送到原 CLI”入口：手机通过 Codex 原生持久队列投递
文字，电脑上正在打开的 CLI 会自动在同一个 thread 继续，不需要退出 CLI，也不会调用恢复或抢占
写入权。模型、工作目录、沙箱和审批策略完全由原 CLI 保持；这个入口暂不支持图片或配置覆盖，
审批仍需在原 CLI 处理。CLI 忙时等待后续处理，已经关闭时要在电脑重新打开原会话才会执行。
“已交给原 CLI 排队”仅表示入队，只有匹配消息回执后才会显示接收和执行状态。未消费消息可以撤回；
已经消费的消息不能通过取消队列停止。发送结果不明、重启或断线时不会自动重复投递。
...
移交接口；原 CLI 使用上述原生队列，已由 Phone Control 持有的 CLI thread 继续使用现场输入。
当前 Windows 实现使用一个受管 stdio App
...
5. 在手机 Phone Control 顶栏点“浏览器”。电脑端切换标签页、导航或刷新完成后，扩展会自动把最新标签和画面同步到手机；手机端仍可直接点按、滑动和输入。需要观看网页视频时，点画面右上角“实时画面”，再点“横向全屏”可获得更大的横屏视图。

Chrome 会显示调试器正在控制标签页，这是扩展使用 Chrome DevTools Protocol 实现截图、实时画面、点按和
输入时的正常安全提示。实时画面只在手机点“实时画面”后开启，退出页面或点“停止实时”即可关闭。
`chrome://`、扩展页和本地 `file://` 页面不会开放给手机。一个时刻只有一台
...
- **浏览器控制**：选择电脑 Chrome 标签页，刷新画面后直接点按或滑动；先点网页输入框，再用下方输入栏发送中文或回车。观看网页视频时可开启实时画面，并配合横向全屏查看。
...
手机交互默认开启。新建会话或开始下一轮时，权限菜单提供五种显式模式：

| 权限模式 | 行为 |
| --- | --- |
| 只读 | 不允许写文件，也不为写入自动提权 |
| 工作区内自动执行 | 可写当前项目；工作区外与网络访问由沙箱阻止 |
| 工作区写入 + 网络 | 可写当前项目并访问网络，适合 `git push`、安装依赖；不会访问工作区外文件，发送前只提示风险 |
| 超出工作区时询问 | 工作区内正常执行；额外命令、文件或 `request_permissions` 请求回到手机单次审批 |
| 完全访问电脑 | 关闭沙箱并自动执行；发送前只提示风险，且仍受管理员策略限制 |

不选择时沿用当前 thread 或本机 Codex 默认，包括已开启的工作区网络权限和完全访问权限；手机端只提示风险，不改变或阻止原权限。所选配置只在 `thread/start` / `turn/start` 生效并延续到后续轮次，不能通过 `turn/steer` 修改。原生审批请求同时绑定 JSON-RPC request、thread 和 turn；Phone Control 只响应由手机开始或追加的 turn，Desktop/CLI 仍使用自己的审批通道。十分钟未处理的手机审批会自动拒绝，轮次结束时遗留审批也会立即失效。
