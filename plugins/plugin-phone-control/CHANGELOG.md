...
- 合并无 `turnId` 的 rollout assistant 片段到后续带身份的 Codex turn，同时保持已完成轮次边界。
- 审批请求在处理中、状态冲突或结果不确定时自动同步手机审批卡，避免拒绝后旧按钮卡死。

## 0.13.2 - 2026-09-21

- 修复并发工具事件覆盖待处理审批的问题；原生 Codex 审批会保持在对应手机任务上，直到明确允许、拒绝或过期。
- 修复手机审批按钮在移动端点击无反馈的问题：审批动作直接绑定到动作区，立即显示送达状态与错误，并避免重复点击。

## 0.13.1 - 2026-09-20

- 修复长轮次详情只请求最近事件时，事件窗口从中间截断导致手机端显示回复却看不到该轮用户提问的问题；现在会保留被截断轮次的所有用户提问。

## 0.13.0 - 2026-09-20

- 修复多机器通过同一 VPS 主机名、不同端口访问时互相覆盖浏览器凭证的问题：每个安装实例现在使用稳定的独立 Cookie 命名空间，并可从旧固定 Cookie 一次性迁移。
- Relay 配置在尚未支持路径前缀时明确拒绝带路径的公网 URL；文档补充同一 VPS 上多台工作站的端口、代理名和主机名隔离要求。
- 后台服务安装现在会验证所选 runtime 的真实 Node 版本，并在写入或重启服务前拒绝 Node 22 以下版本。

## 0.12.2 - 2026-09-04

- 将结构化结果附录移动到对应对话轮次的最终回复之后，默认折叠并仅显示文件、验证、命令和警告等补充信息，避免重复展示最终回复。
- 每个已结束轮次独立生成附带信息卡；卡片名称默认由 Codex 根据任务上下文智能概括，并持久化保存，手动命名仍优先。
- 修复长会话的附带信息只剩最新一轮：结果快照不再受 240 条事件滚动窗口影响，并按完成事件与轮次顺序稳健绑定到对应卡片。
- 修复详情页渲染时先读取后声明轮次状态导致所有会话无法打开的问题；前端资源升级到 `v91`，并新增发布门禁检查声明顺序。
