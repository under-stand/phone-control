...
手机查看的任务流，并支持行动收件箱、统一指令状态、结构化结果、回答问题、续聊、发送图片、中止任务、
新建会话、浏览器控制和完成提醒。

任务卡片的智能名称按需生成：默认处理最近列表中可见的前 12 个用户会话；切换到历史、全部或搜索结果后，
对应可见的历史会话同样有效。生成成功会立即更新并持久化，不需要刷新浏览器；暂时失败时保留本地候选，
后续列表刷新会重试。详情里的“智能生成”只填入可编辑候选，点击“保存”后才会改名。

刷新页面或从后台回来时，列表会先显示浏览器本地保存的会话摘要，再由实时连接校正；缓存不含完整对话，
并会显示“正在同步”，不会把旧的工作状态误认为实时状态。
