...
- 修复控制通道的所有权边界：桥接层不再对已加载的 CLI/Desktop 线程隐式调用 `thread/resume`，只有手机创建的会话会被自动恢复，避免与 Codex CLI 0.147+ 的单写者约束冲突。
- v0.13 稳定性切片：新增显式连接状态机，区分实时 SSE、HTTP 已同步、后台暂停和真正失联；刷新时先展示有界会话摘要缓存，再由实时同步校正。
- 智能任务命名失败后可在冷却后重试，历史会话在用户进入历史、搜索或展开时按需命名。
